...
每个房间连接着一个**壁橱（closet）**，这就是有趣的地方。我们开发了一种名为 **AAAK** 的 AI 语言。别问——这本身就是一个完整的故事。你的代理每次唤醒时都会学习 AAAK 简写。因为 AAAK 本质上是英语，但是一个非常截断的版本，你的代理在几秒钟内就能理解如何使用它。它作为安装的一部分，内置在 MemPalace 代码中。在下一次更新中，我们将把 AAAK 直接添加到壁橱中，这将是一个真正的游戏规则改变者——壁橱中的信息量会大得多，但占用的空间和代理的阅读时间会少得多。

壁橱里面有**抽屉（drawer）**，那些抽屉就是你原始文件的存放处。在第一个版本中，我们没有将 AAAK 用作壁橱工具，但即便如此，摘要在我们跨多个基准测试平台所做的所有基准测试中显示了 **96.6% 的召回率**。一旦壁橱使用 AAAK，搜索将更快，同时保持每个字精确不变。但即使现在，壁橱方法对于在狭小空间中存储多少信息也是一个巨大的助力——它用于轻松地将你的 AI 代理指向原始文件所在的抽屉。你永远不会丢失任何东西，这一切在几秒钟内完成。
...
创建专注于特定领域的代理。每个代理在宫殿中获得自己的翼和日记——不在你的 CLAUDE.md 中。添加 50 个代理，你的配置大小保持不变。
...
```
你有 MemPalace 代理。运行 mempalace_list_agents 来查看它们。
```
...
每个代理是你数据上的专业镜头。审查者记住它见过的每一个 bug 模式。架构师记住每一个设计决策。运维代理记住每一个事件。它们不共享草稿本——它们各自维护自己的记忆。

Letta 对代理管理记忆收取 $20–200/月。MemPalace 用一个翼就做到了。
